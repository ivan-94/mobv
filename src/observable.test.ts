...
import { isReactive, isShallow, watchEffect, isVue2 } from 'vue-demi';
import { collectAnnotations } from './decorators';
import { makeObservable } from './makeObservable';
import { observable } from './observable';
import { override } from './override';

const decorators: { d: PropertyDecorator; name: string }[] = [
  { d: observable, name: 'observable' },
  { d: observable.ref, name: 'observable.ref' },
  { d: observable.shallow, name: 'observable.shallow' },
];
...
describe('注解', () => {
  test('注解应该能正常设置', () => {
    ...
    expect(Object.keys(collectAnnotations(Test.prototype))).toEqual([
      'a',
      'b',
      'c',
    ]);
    expect(
      Object.values(collectAnnotations(Test.prototype)).map(
        i => i.options?.type,
      ),
    ).toEqual(['deep', 'ref', 'shallow']);
  });
...
describe('observable 数据响应', () => {
  ...
  test.each(decorators)('字段重新赋值应该能够响应: $name', ({d}) => {
    class Base {
      @d
      count = 1;
