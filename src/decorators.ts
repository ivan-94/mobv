...
export function storeAnnotation(
  ...
  if (process.env.NODE_ENV !== 'production') {
    // 检查是否覆盖
    if (
      isOverridden(annotation) &&
      !hasProp(prototype[storedAnnotationSymbol], key)
    ) {
      const fieldName = `${
        prototype.constructor.name
      }.prototype.${key.toString()}`;
      throw new Error(
        `${fieldName} 使用了 @override, 但是当前类或父类未定义该字段`,
      );
    }
    ...
    if (
      ...
      const prevAnnotationType = getAnnotationType(
        prototype[storedAnnotationSymbol][key],
      );
...
export function collectAnnotations(target: any): AnnotationMap {
  if (!hasProp(target, storedAnnotationSymbol)) {
    ...
    // 继承注解
    addHiddenProp(target, storedAnnotationSymbol, {
      ...target[storedAnnotationSymbol],
    });
  }
 ...
 * @returns
 */
export function createAnnotationDecorator(
  name: string,
  annotation: Annotation,
): PropertyDecorator {
